...
int main()
{
    ...
    for(int i = 0; i < 27; i++)
    {
        if(arr_big[i] != 0)
        {
            arr_small[count2++] = arr_big[i];	/*将arr_big中统计的字母次数放入数组arr_small中*/
        }
    }
    ...
    {
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1)  /*找到不存在左右子树节点的节点*/
        {
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1)  /*如果不存在权重和父结点就停止*/
        {
            break;
        }
        if(huffman[i][3] < min && huffman[i][0] == -1)  /*找出权重最小的结点，作为左节点*/
        {
            min = huffman[i][3];
            ii[0] = i;
        }
...
![屏幕截图 2022-11-05 214817](C:\Users\86180\Desktop\屏幕截图 2022-11-05 214817.png)
